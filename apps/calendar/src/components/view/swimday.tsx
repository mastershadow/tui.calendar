...
import type { Swimlane, Swimlanes, WeekOptions } from '@t/options';

import { SwimGridRow } from '../dayGridWeek/swimGridRow';
...
export function SwimDay() {
  ...
  const { narrowWeekend, startDayOfWeek, workweek, hourStart, hourEnd, taskView } = weekOptions;
  ...
  const dayGridEvents = useMemo(() => {
    ...
    const additionalPanels = (taskView as Swimlanes).swimlanes.map((s) => s.id);

    return getWeekViewEvents(
      days,
      calendarData,
      {
        narrowWeekend,
  ...
  }, [calendarData, days, hourEnd, hourStart, narrowWeekend, primaryTimezoneName, taskView]);
  ...
  const activePanels = getActivePanels(taskView, false);
  const gridRows = activePanels.map((key) => {
    const sl = key as Swimlane;
    const rowType = sl.id;
